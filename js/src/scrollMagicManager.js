define('scrollmagicmanager',['jquery','TweenMax','ScrollMagic','ScrollMagicAddIndicators','ScrollMagicJquery','utils','ScrollMagicGSAP'],
	...
	Scheme.addInViewPortDetectScrollMagic = function(data,eventHandle){

		var tmpDuration = data.vh - data.triggerDom.height() * (2/3) - 118,
			tmpOffset = data.triggerDom.height() * (7/9);
		if(tmpDuration <= 100){
			tmpDuration = 100;
		}
        ...
        tmpScene.on('progress',function(e){
	    	...
	    	eventHandle.inViewportProgress(e.progress);
	    });
	    var tmpInViewPortDetectObj = {
        	'scene':tmpScene,
        	'data':data
        };
	    inViewportScene.push(tmpInViewPortDetectObj);
	...
	Scheme.update = function(tmpW,tmpH){
		...
		for(var a = 0,b = inViewportScene.length;a<b;a++){
			var tmpDuration = tmpH-inViewportScene[a].data.triggerDom.height() * (2/3) - 118,
				tmpOffset = inViewportScene[a].data.triggerDom.height() * (7/9);
			if(tmpDuration <= 100){
				tmpDuration = 100;
			}

            inViewportScene[a].scene.duration(tmpDuration);
            inViewportScene[a].scene.offset(tmpOffset);
		}
